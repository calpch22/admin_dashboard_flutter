...
class UserFormProvider extends ChangeNotifier {
  ...
  Future<Usuario> uploadImageVideo( String path, Uint8List bytes, String filename ) async {

    try {
      
      final resp = await CafeApi.uploadFileVideo(path, bytes, filename);
      user = Usuario.fromMap(resp);
      notifyListeners();
...
/*
  Future<Usuario> uploadImageVideo(String path, PlatformFile file) async {
    try {
      final resp = await CafeApi.uploadFileVideo(path, file);
      user = Usuario.fromMap(resp);
      notifyListeners();
      ...
      print(e);
      throw 'Error en user from povider image';
    }
  }
*/
}
